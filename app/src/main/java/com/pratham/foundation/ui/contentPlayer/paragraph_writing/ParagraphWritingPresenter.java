        ...
        if (questionModel != null && !questionModel.isEmpty()) {
            for (int i = 0; i < questionModel.size(); i++) {
                if (checkIsAttempted(questionModel.get(i))) {
                    ...
                    newResId = GameConstatnts.getString(resId, jsonName, questionModel.get(i).getQid(), questionModel.get(i).getUserAnswer(), questionModel.get(i).getQuestion(), questionModel.get(i).getPhotourl());
