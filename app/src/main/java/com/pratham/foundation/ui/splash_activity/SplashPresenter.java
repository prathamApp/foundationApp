        ...
        try {
            ...
            if (!dbExist) {
                ...
                try {
                    ...
                    if (new File(FC_Utility.getStoragePath().getAbsolutePath() + "/PrathamBackups/" + DB_NAME).exists())
                        copyDb = true;
                    getSdCardPath();
                } catch (Exception e) {
                    e.printStackTrace();
...
//                        ArrayList<String> sdPath = FileUtils.getExtSdCardPaths(context);
            SQLiteDatabase db = SQLiteDatabase.openDatabase(FC_Utility.getStoragePath()
...
//                                    detail.setProgramid("" + content_cursor.getString(content_cursor.getColumnIndex("programid")));
    ...
    public void copyDBFile() {
        ...
        try {
            File internalDB = new File(FC_Utility.getStoragePath().toString() + "/.FCAInternal/" + DB_NAME);
        ...
        try {
            File direct = new File(FC_Utility.getStoragePath().toString() + "/.FCAInternal");
            if (!direct.exists()) direct.mkdir();
            InputStream in = new FileInputStream(ApplicationClass.contentSDPath + "/.FCA/" + DB_NAME);
            OutputStream out = new FileOutputStream(FC_Utility.getStoragePath().toString() + "/.FCAInternal/" + DB_NAME);
            byte[] buffer = new byte[1024];
            int read = in.read(buffer);
            while (read != -1) {
                out.write(buffer, 0, read);
...
/*        if (new File(ApplicationClass.contentSDPath + "/.FCA/TestJsons/Test_" + no + ".json").exists()) {
            try {
                File internalTestJson = new File(FC_Utility.getStoragePath().toString()
            ...
            try {
                File direct = new File(FC_Utility.getStoragePath().toString() +
                        "/.FCAInternal/TestJsons");
                if (!direct.exists()) direct.mkdir();
                InputStream in = new FileInputStream(ApplicationClass.contentSDPath + "/.FCA/TestJsons/Test_" + no + ".json");
                OutputStream out = new FileOutputStream(FC_Utility.getStoragePath().toString()
                        + "/.FCAInternal/TestJsons/" + "Test_" + no + ".json");
                byte[] buffer = new byte[1024];
                int read = in.read(buffer);
                while (read != -1) {
        ...
        new AsyncTask<Void, Integer, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                ...
                if (!FastSave.getInstance().getBoolean(FC_Constants.INITIAL_SD_COPIED, false))
                    try {
                        File db_file;
                        db_file = new File(FC_Utility.getStoragePath().toString() + "/.FCAInternal/" + DB_NAME);
                        if (db_file.exists()) {
                            SQLiteDatabase db = SQLiteDatabase.openDatabase(db_file.getAbsolutePath(), null, SQLiteDatabase.OPEN_READONLY);
                            if (db != null) {
                                Cursor content_cursor;
        ...
        try {
            ...
            if (!FastSave.getInstance().getBoolean(FC_Constants.KEY_MENU_COPIED, false)) {
                ...
                if (ApplicationClass.getAppMode()) {
                    Log.d("copyDBFile", "Exception : ");
                    copyDBFile();
                    folder_file = new File(FC_Utility.getStoragePath().toString() + "/.FCAInternal");
                } else
                    folder_file = new File(ApplicationClass.foundationPath + "/.FCA/");
                if (folder_file.exists()) {
...
//                                        detail.setProgramid(content_cursor.getString(content_cursor.getColumnIndex("programid")));
    ...
    public boolean getSdCardPath() {
        ...
        if (base_path.size() > 0) {
            ...
            fpath = FC_Utility.getStoragePath().getAbsolutePath();
