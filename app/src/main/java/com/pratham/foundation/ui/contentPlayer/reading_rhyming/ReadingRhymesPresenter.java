...
public class ReadingRhymesPresenter implements ReadingRhymesContract.ReadingRhymesPresenter {
    ...
    public void getLearntWordsCount() {
        learntWordCount = AppDatabase.getDatabaseInstance(context).getKeyWordDao().checkWordCount(FastSave.getInstance().getString(FC_Constants.CURRENT_STUDENT_ID, "NA"), "" + resId);
    }
    ...
    private boolean checkWord(String wordStr) {
        try {
            String word = AppDatabase.getDatabaseInstance(context).getKeyWordDao().checkWord(FastSave.getInstance().getString(FC_Constants.CURRENT_STUDENT_ID, "NA"), "" + resId, wordStr.toLowerCase());
            return word != null;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
